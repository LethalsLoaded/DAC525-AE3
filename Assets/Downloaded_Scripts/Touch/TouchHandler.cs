...
public class TouchHandler : MonoBehaviour
{
    ...
    [Header("Options")] [Space(10, order = 2)]
    ...
    private static float _screenPixelsPerCm;
...
// iPhone 4-6 range
    ...
    public void AssignTouches()
    {
        ...
        else
        {
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                touches = new[] {fakeTouch, simulatePinch};
            }
    ...
    private void Update()
    {
        ...
        if (_touchCache.Count > 0)
            for (var i = _touchCache.Count - 1; i >= 0; i--)
    ...
    private void OnGUI()
    {
        if (displayStatsInEditor)
        {
            ...
            var rect = new Rect(5, 5, w, h / 50);
            style.alignment = TextAnchor.UpperLeft;
            style.fontSize = h / 50;
            style.normal.textColor = fontColor;

            var text = string.Format("Is Pinching:\t{0}", Pinch.active);
            ...
            for (var i = 0; i < _touchCache.Count; i++)
            {
                ...
                GUI.DrawTextureWithTexCoords(
                    new Rect(_touchCache[i].currentPos.x - t.width / 2,
                        Screen.height - _touchCache[i].currentPos.y - t.height / 2, tr.width, tr.height), t, r);
            }
        }
    }
#endif
    ...
    {
        foreach (var t in _touchCache) t.CheckRayHit(layerMask);
    ...
    {
        foreach (var t in _touchCache) t.CheckRayHit2D(layerMask);
        return _touchCache.ToArray();
    }
    ...
    public static class DoubleSwipe
    {
        public static directions Direction()
        {
            if (_touchCache.Count == 2 &&
                _touchCache[0].action == actions.Swipe &&
                _touchCache[1].action == actions.Swipe)
    ...
    public static class Pinch
    {
        ...
        public static void Update()
        {
            ...
            if (active)
            {
                ...
                {
                    if (Math.Abs(deltaDistance) > _dragThreshold)
        ...
        {
            if (_touchCache.Count == 2)
                return _touchCache[1].action != actions.Swipe &&
                       _touchCache[0].action != actions.Swipe && _touchCache[0].totalPressTime > pinchDelay &&
        ...
        public static bool Raycast3D(out RaycastHit hit, int[] layerMask)
        {
            ...
            if (active)
            {
                ...
                var tempMask = 0;
                foreach (var t in layerMask) tempMask |= 1 << t;
